...
public static function handleDeletedImage($data, $field, $deletePath)
{
    if (is_array($data[$field])) {
        foreach ($data[$field] as $filename) {
            self::deleteImage($filename, $deletePath);
        }
    } elseif ($data[$field] != null) {
        self::deleteImage($data[$field], $deletePath);
    }
}

private static function deleteImage($filename, $deletePath){
    $deleteFilePath = public_path($deletePath . $filename);
    if (file_exists($deleteFilePath)) {
        unlink($deleteFilePath);
    }
}
